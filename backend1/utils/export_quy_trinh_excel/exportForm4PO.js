...
const TEMPLATE_FILE = 'form_mau_4_PO.xlsx';
const HEADERS = [
	'Mã POT',
...
function getCellValue(fields, header) {
	const value = getField(fields, header);
	if (header === 'Ngày giờ muốn nhận hàng') return formatDateTime(value);
	return value;
}
...
async function createForm4POFile(records) {
	...
	data.forEach((record, index) => {
		...
		HEADERS.forEach((header, columnIndex) => {
			row.getCell(columnIndex + 1).value = getCellValue(fields, header);
		});
	});
	...
	worksheet.autoFilter = { from: 'A1', to: `R${data.length + 1}` };
	return workbook;
}

module.exports = { createForm4POFile };

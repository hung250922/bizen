...
function LamThucDonTuanPage() {
  ...
  useEffect(() => {
    async function boot() {
      const [clientRes, bomRes, priceRes, seasonRes, comboRes, scoreRes] = await Promise.all([
  ...
  const catalog = useMemo(() => {
    return DANH_MUC_MON_AN.map((item) => {
      const bom = boms.find((row) => row.food_name === item.name) || {}
      return { ...item, bom, cost: bomCost(bom, priceMap) }
    })
  }, [boms, priceMap])
...
const proteinGroups = groupBy(filteredCatalog, 'protein')
  return (
    <main className="mm">

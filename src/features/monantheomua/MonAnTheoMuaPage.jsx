...
function MonAnTheoMuaPage() {
  ...
  const visible = rows.filter((row) => Boolean(row.is_encouraged) === encouraged)
  ...
  async function save(event) {
    ...
    <main className="season-page">
      ...
      <div className="season-tabs">
        <button type="button" className={encouraged ? 'active' : ''} onClick={() => setTab('plus')}>
          Món ăn khuyến khích
        </button>
        <button type="button" className={!encouraged ? 'active' : ''} onClick={() => setTab('minus')}>
          Món ăn loại trừ
        </button>
      </div>
      <div className="season-layout">
        ...
        <section className="season-list">
          <h2>{encouraged ? 'Danh sách món khuyến khích' : 'Danh sách món loại trừ'}</h2>
          <table>
            <thead>

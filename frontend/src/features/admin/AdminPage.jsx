...
function mapUser(user) {
  ...
  return {
    ...
    permissions: admin ? allTabPermissionKeys : (Array.isArray(user.permissions) ? user.permissions : ['baosoluongkhachhang']),
  }
}
...
function AdminPage() {
  ...
  const filteredUserTotal = users.filter((user) => {
  ...
  const filteredUsers = useMemo(() => users.filter((user) => {
    ...
    const matchesRole = roleFilter === 'all' || user.role === roleFilter
    return matchesQuery && matchesStatus && matchesRole
  }).slice((page - 1) * 25, page * 25), [page, query, roleFilter, status, users])
  ...
  useEffect(() => {
    document.querySelectorAll('.avatar-wrap img').forEach((image) => {
  ...
  async function updateRole(id, role) {
    try {
      const permissions = role === 'admin' ? allTabPermissionKeys : ['dashboard']
      await httpClient.put(`/user/${id}`, { scope: role, ...(permissions ? { permissions } : {}) })
  ...
  async function confirmAction() {
    ...
    try {
      if (pendingAction.type === 'delete') {
        ...
        await httpClient.put(`/user/${pendingAction.user.id}`, { isAuthenticated, approvalStatus: isAuthenticated ? 'approved' : 'blocked' })
        setUsers((current) => current.map((user) => user.id === pendingAction.user.id ? { ...user, locked: !isAuthenticated, pendingApproval: false, online: isAuthenticated } : user))
      }
    } catch (requestError) {

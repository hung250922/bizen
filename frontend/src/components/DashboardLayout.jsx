...
import { AUTH_KEY, LAST_ROUTE_KEY } from '../features/auth/AuthPage'
...
function DashboardLayout() {
  ...
  useEffect(() => {
    ...
    presenceStoppedRef.current = false
    const sendPresence = () => {
      if (presenceStoppedRef.current) return
      httpClient.post(`/user/${userId}/presence`, { isOnline: true }).catch(() => {})
    }
    const markOfflineOnPageHide = () => {
      ...
      fetch(`${httpClient.defaults.baseURL || ''}/user/${userId}/presence`, {
        method: 'POST',
        headers: {
    ...
    sendPresence()
    presenceTimerRef.current = window.setInterval(sendPresence, 30000)
  ...
  async function handleLogout() {
    ...
    presenceTimerRef.current = null
    if (currentUser?._id) {
      await httpClient.post(`/user/${currentUser._id}/presence`, { isOnline: false }).catch((error) => {
        console.error('Không cập nhật được trạng thái offline:', error)
      })
    }
